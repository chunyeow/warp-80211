...
int xilnet_ip(unsigned char*buf, int len, unsigned int eth_dev_num) {
    ...
    if ((buf+ETH_HDR_LEN)[IP_DADDR_BASE]    == eth_device[eth_dev_num].node_ip_addr[0] &&
        (buf+ETH_HDR_LEN)[IP_DADDR_BASE+1]  == eth_device[eth_dev_num].node_ip_addr[1] &&
        (buf+ETH_HDR_LEN)[IP_DADDR_BASE+2]  == eth_device[eth_dev_num].node_ip_addr[2] &&
        ((buf+ETH_HDR_LEN)[IP_DADDR_BASE+3] == eth_device[eth_dev_num].node_ip_addr[3] || (buf+ETH_HDR_LEN)[IP_DADDR_BASE+3] == 255 )) {
