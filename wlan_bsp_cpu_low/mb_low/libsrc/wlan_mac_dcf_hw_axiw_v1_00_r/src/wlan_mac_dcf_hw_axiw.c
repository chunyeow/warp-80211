...
xc_status_t xc_wlan_mac_dcf_hw_axiw_getshmem(xc_iface_t *iface, const char *name, void **shmem)
{
    ...
    if (strcmp("RX_START_TIMESTAMP_MSB", name) == 0) {
    ...
    } else if (strcmp("TIMESTAMP_COUNTER_MSB", name) == 0) {
        *shmem = (void *) & _config_table->TIMESTAMP_COUNTER_MSB;
    } else if (strcmp("TIMESTAMP_COUNTER_LSB", name) == 0) {
        *shmem = (void *) & _config_table->TIMESTAMP_COUNTER_LSB;
